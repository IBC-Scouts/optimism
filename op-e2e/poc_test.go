...
func TestIBCCrossChainMessenger(t *testing.T) {
	InitParallel(t)
	...
	tx, err = transactions.PadGasEstimate(opts, 1.1, func(opts *bind.TransactOpts) (*types.Transaction, error) {
		return ibcMessenger.SendMessage(opts, weth9Address, []byte("hello cosmos"), 100000)
	})
	require.NoError(t, err)
	...
	depositEvent, err := receipts.FindLog(ibcMsgReceipt.Logs, portal.ParseTransactionDeposited)
